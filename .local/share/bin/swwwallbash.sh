...
set -a
source "${wallbashOut}"
if [ -f "${themeDir}/theme.dcol" ] && [ "${enableWallDcol}" -eq 0 ]  ; then
    source "${themeDir}/theme.dcol"
    echo "[theme] Overriding dominant colors from \"${myTheme}\""
    echo "[note] Remove \"${themeDir}/theme.dcol\" to use wallpaper dominant colors"
fi
...
if [ "${enableWallDcol}" -eq 0 ] && [[ "${reload_flag}" -eq 1 ]] ; then
    ...
    echo ":: deploying ${myTheme} colors :: ${dcol_mode} wallpaper detected"
    mapfile -d '' -t deployList < <(find "${themeDir}" -type f -name "*.theme" -print0)

    while read -r pKey ; do
        fKey="$(find "${themeDir}" -type f -name "$(basename "${pKey%.dcol}.theme")")"
...
if [ "${enableWallDcol}" -eq 0 ]; then
    colorScheme="$({ grep -q "^[[:space:]]*\$COLOR-SCHEME\s*=" "${themeDir}/hypr.theme" && grep "^[[:space:]]*\$COLOR-SCHEME\s*=" "${themeDir}/hypr.theme" | cut -d '=' -f2 | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' ;} || 
